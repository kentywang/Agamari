...
class Canvas extends Component {
    ...
    render = () => {
        let { players, gameState, keepPlaying, casualty, abilities } = this.props;
        let { leaderboard, displayVol, instructions } = this.state;
        ...
        <div className="hud">
          <div ref="leaderboard" className="leaderboard">
            <table>
              <tbody>
              {player && leaderboard && leaderboard.map((person, index) => (<tr key={`leaderboard-row-${index + 1}`} style={myNick === person.nick ? { backgroundColor: 'rgba(0,0,0,0.3)' } : {}}>
