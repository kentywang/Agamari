...
class Chat extends Component {
    ...
    componentDidMount() {
        ...
        window.addEventListener('keydown', evt => {
            ...
            if (!isChatting && evt.keyCode == 13)
                start();
            if (isChatting && evt.keyCode == 27)
                stop();
        });
    }
    render() {
      ...
      {gameState.isChatting ?
                <div id="message-box">
            <ul ref="messageBox" id="message-list" className="collection">
            ...
            <input ref="chatInput" value={message} onChange={updateMessage} onBlur={stop} onKeyPress={evt => { if (evt.key === 'Enter')
                    sendMessage(); }} maxLength={70} type="text" id="new-message" placeholder="press 'esc' to close"/>
          </div> :
                <div>
            <div id="last-message" onClick={start}>
                {lastMessage &&
