...
import { scene, camera, world, groundMaterial, ballMaterial } from './main';
...
export class Player {
    ...
    init() {
            ...
            this.mesh.cannon.addEventListener('collide', e => {
                ...
                if (player && Date.now() - lastEaten > 5000) {
                    ...
                    for (let contact of world.contacts) {
                        let thisHits = contact.bi === this.mesh.cannon, mainIsHit = contact.bj === player.cannon, mainHits = contact.bi === player.cannon, thisIsHit = contact.bj === this.mesh.cannon;
        ...
        if (isMainPlayer) {
            controls = new THREE.PlayerControls(camera, this.mesh, this.mesh.cannon, id);
        }
    }
    get meshData() {
        return {
