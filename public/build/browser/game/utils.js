import { scene, world } from '../game/main';
...
export const makeTextSprite = (message, fontsize) => {
    let ctx, texture, sprite, spriteMaterial, canvas = document.createElement('canvas');
...
export const attachFood = (id, playerId, playerData) => {
    ...
    if (foodObject) {
        ...
        let vec1 = new CANNON.Vec3((foodObject.position.x - playerData.x) * scaled, (foodObject.position.z - playerData.z) * scaled, (foodObject.position.y - playerData.y) * scaled);
        ...
        let vec2 = new THREE.Vector3((foodObject.position.x - playerData.x) * scaled, (foodObject.position.y - playerData.y) * scaled, (foodObject.position.z - playerData.z) * scaled);
...
export const attachPlayer = (id, playerId, eaterData, eatenData) => {
    let foodObject = scene.getObjectByName(id);
    let player = scene.getObjectByName(playerId);
    let newQuat = new CANNON.Quaternion(-eaterData.qx, -eaterData.qz, -eaterData.qy, eaterData.qw);
    let newEatenQuat = new CANNON.Quaternion(-eatenData.qx, -eatenData.qz, -eatenData.qy, eatenData.qw);
    let threeQuat = new THREE.Quaternion(eaterData.qx, eaterData.qy, eaterData.qz, eaterData.qw);
    ...
    if (foodObject) {
        ...
        // figure out where and in what orientation to attach object to player
        let vec1 = new CANNON.Vec3((eatenData.x - eaterData.x) * scaled, (eatenData.z - eaterData.z) * scaled, (eatenData.y - eaterData.y) * scaled);
        let vmult = newQuat.inverse().vmult(vec1);
        player.cannon.addShape(foodObject.cannon.shapes[0], vmult, newQuat.inverse());
        let invQuat = threeQuat.inverse();
        let vec2 = new THREE.Vector3((eatenData.x - eaterData.x) * scaled, (eatenData.y - eaterData.y) * scaled, (eatenData.z - eaterData.z) * scaled);
        let vecRot = vec2.applyQuaternion(invQuat);
