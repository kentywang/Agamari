    ...
    let { user } = store.getState();
      ...
      if (!isMainPlayer) {
        this.mesh.cannon.addEventListener('collide', e => {
          ...
          let player = scene.getObjectByName(user.id.toString());
          // cooldown timer for being eaten
          if (player && ((Date.now() - this.lastEaten) > 3000)) {
